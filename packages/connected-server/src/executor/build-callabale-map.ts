...
export default function buildCallableMap(callables: Callable[]): CallableMap {
  ...
  const collision = Array.from(map.values()).find(
    (group) => group.length !== 1
  );
  if (collision) {
    const name = collision[0].property
      ? `${collision[0].name}.${collision[0].property}`
      : collision[0].name;
    throw new TypeError(
      `Name conflicts found for "${name}". Definitions found in ${collision
        .map((c) => c.file)
        .join(', ')}`
    );
  }
