...
describe('Middleware', () => {
  ...
  it('invokes handlers in order', async () => {
    const request = {};
    const middleware = new Middleware();
    middleware.use((next, request) => { request.x = 1; next(); });
    middleware.use((next, request) => { request.x = 2; next(); });
    await new Promise(resolve => middleware.go(resolve, request));
  ...
  it('invokes async handler', async () => {
    ...
    const middleware = new Middleware();
    middleware.use((next, request) => {
      setTimeout(() => { request.x = 1; next(); }, 100);
    });
    await new Promise(resolve => middleware.go(resolve, request));
    expect(request).toEqual({ x: 1 });
  });
  ...
  it('returns no error', async () => {
    ...
    const error = await new Promise(resolve => middleware.go(resolve, request));
  ...
  it('no error handlers called in normal flow', async () => {
    ...
    const middleware = new Middleware();
    middleware.use(next => next());
    middleware.useErrorHandler((error, next, request) => { request.x = 1; next(error); });
    const error = await new Promise(resolve => middleware.go(resolve, request));
    expect(request).toEqual({});
    expect(error).toBeUndefined();
  });
